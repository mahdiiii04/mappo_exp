...
from scenarios.simple_spread_pref import Scenario as PrefSpreadScenario
...
def make_env(cfg, seed: int) -> TransformedEnv:
    """Construct a VmasEnv using the custom preference-spread scenario."""
    scenario = PrefSpreadScenario()
    env = VmasEnv(
        scenario=scenario,
        ...
        seed=seed,
        # Scenario kwargs forwarded to make_world
        n_agents=cfg.env.scenario.n_agents,
        pref_bonus=cfg.env.scenario.pref_bonus,
        coverage_radius=cfg.env.scenario.coverage_radius,
        collision_penalty=cfg.env.scenario.collision_penalty,
        pref_switch_interval=cfg.env.scenario.get("pref_switch_interval", None),
    )
    env = TransformedEnv(
        env,
        RewardSum(
...
def evaluate_policy(env_test, policy) -> float:
    policy.eval()
...
def train(cfg: DictConfig):
    ...
    log_dir = os.path.join(
        "tb_logs", f"pref_spread-seed-{cfg.seed}"
    )
    ...
    env_test = make_env(cfg, seed=cfg.seed + 1000)
    ...
    collector = Collector(
        ...
        postproc=DoneTransform(
            reward_key=env.reward_key, done_keys=env.done_keys
        ),
    )

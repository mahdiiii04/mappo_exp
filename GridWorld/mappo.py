...
def train(cfg: DictConfig):
    ...
    # How close to the pre-switch baseline counts as "recovered":
    #   mean_episode_reward >= RECONVERGE_THRESHOLD * pre_switch_reward
    # We require RECONVERGE_PATIENCE consecutive iterations above the threshold
    # to avoid noisy spikes being mistaken for genuine recovery.
    RECONVERGE_THRESHOLD = 0.95
    RECONVERGE_PATIENCE  = 3

    prev_mean_reward  = None   # reward from the immediately preceding iteration
    prev_pos_entropy  = None   # entropy from the immediately preceding iteration

    _switch_baseline_reward  = None  # pre-switch snapshot
    _switch_baseline_entropy = None
    _switch_frame            = None  # global_step at the switch
    _switch_iter             = None  # loop index i at the switch
    ...
    for i, tensordict_data in enumerate(collector):
        ...
        if goal_changed:
            ...
            # Baseline = the iteration that ended *just before* the switch.
            # prev_mean_reward is None only on the very first iteration.
            ...
            # Immediate reward drop at the switch moment
        ...
        if _is_post_switch and _switch_baseline_reward is not None:
            ...
            # Patience-gated reconvergence event
        ...
        metric_history.append({
            ...
            # Reconvergence: non-None only while tracking a post-switch window
            "reward_gap":           (_switch_baseline_reward - mean_episode_reward)
                                    if _is_post_switch and _switch_baseline_reward is not None
                                    else None,
            "frames_since_switch":  (global_step - _switch_frame)
        ...
        # Roll per-iteration state for the next iteration's baseline snapshot
